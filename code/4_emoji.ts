type emojiList = {[name: string]: string};

function searchEmoji(name: string, emojiList: emojiList): {code: string, isCustom: boolean} {
    let code: string = name;

    while (true) {  // alias解消するまで
        if (code in emojiList) {
            code = emojiList[code];
            if (code.indexOf("alias:") == 0) {  // alias
                code = code.slice(6);
            } else {
                break;
            }
    ...
    let isCustom: boolean = (code.indexOf("https://") == 0);
    
    return {code, isCustom};
}


function createEmojiList(token: string): emojiList {
    // 組み込み絵文字のリスト
    ...
    let customEmojiList: emojiList = emojiListAPI(token);
    emojiList = Object.assign(emojiList, customEmojiList);
